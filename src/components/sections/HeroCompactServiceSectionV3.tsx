...
import type { HeroCompactServiceLayout } from "@/content/section-style-options";

export type HeroCompactServiceSectionV3Props = {
  align?: HeroCompactServiceLayout;
  body: string;
  cardBorder?: "on" | "off";
  cardFill?: "solid" | "none";
  ctaBody: string;
...
// `layout` positions the CTA panel across the 14-column grid. `none` is the
// intentional no-image composition: copy grows to eight columns and the CTA
// grows to five, with one column of breathing room between them.
const layoutColumnClassName: Record<
  HeroCompactServiceLayout,
  { cta: string; image: string; text: string }
> = {
  none: {
    cta: "col-span-5 col-start-10",
    image: "",
    text: "col-span-8 col-start-1",
  },
  center: {
...
/**
 * REVEAL ORDER, COMPUTED FROM `layout` RATHER THAN FROM JSX ORDER.
 *
 * The three blocks are written copy-image-cta. Center puts the CTA between copy
 * and image; right trails it. `none` keeps that right-layout reading order but
 * omits the media unit, so its CTA closes the sequence one step earlier.
 *
...
> = {
  none: { heading: 0, body: 1, image: 2, cta: 2 },
...
export function HeroCompactServiceSectionV3({
  ...
  const columns = layoutColumnClassName[layout];
  const order = revealIndex[layout];
  const hasImage = layout !== "none";
        ...
        >
          <div className="fluid-type-frame w-full">
            ...
            >
              ...
              <HeadingTag
                className={cx(
                  layout === "none" ? "type-display-lg" : "type-heading-xl",
