        ...
        <div className="pointer-events-none relative z-30 grid grid-cols-[1fr_auto_1fr] items-center px-8 py-3 max-lg:hidden">
          <ul className="pointer-events-auto col-start-2 flex min-h-12 items-center gap-1 rounded-lg border border-service-border bg-white/90 p-1 text-sm font-semibold text-service-ink shadow-service backdrop-blur-md">
            {links.map((link) => {
              ...
              const menuId = `floating-nav-${link.label.toLowerCase().replaceAll(" ", "-")}`;
                  ...
                  {hasDropdown ? (
                    <>
                      <button
                        ...
                        className="flex h-10 cursor-pointer items-center gap-2 rounded-md px-4 transition-colors hover:bg-service-surface hover:text-service-accent"
                        type="button"
                        onClick={() =>
                      ...
                      <AnimatePresence initial={false}>
                        {isOpen ? (
                          <motion.div
                            id={menuId}
                            className="absolute left-0 top-[calc(100%+0.5rem)] z-40 w-56 rounded-lg border border-service-border bg-white p-2 shadow-service"
                            initial={{
                              opacity: 0,
                    ...
                    <a
                      className="flex h-10 cursor-pointer items-center rounded-md px-4 transition-colors hover:bg-service-surface hover:text-service-accent"
                      href="#"
                    >
                      {link.label}
                    </a>
