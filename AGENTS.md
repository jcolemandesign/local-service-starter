...
- wired into **all three render paths** — `renderPageTemplateSection` in `PageTemplatePreview.tsx` (with a `xxxProps()` mapper, for staged pages), `renderPreviewSection` in `PagebuilderSection.tsx` (the gallery; a missing case renders the "Preview unavailable" placeholder), and the ternary chain in `PagebuilderShell.tsx` (the builder canvas). Any section reading `cardFill`/`cardBorder`/`icons`/`cardLinks`/an align axis **must** have its own branch in that third chain passing the `getSection*` resolvers — its fallback renders a statically built element with no toggle values, so the controls appear and silently do nothing
